...
7. доставляє кожен `Running` tick в один fixed-dispatch `.my` вираз з
   `scripts/dispatcher.lisp`; C++ не викликає `(гравець-присутній?)`, не
...
можуть з'являтися лише після `Running` tick і залежать від обраного `.lisp`
...
host-фактом «гравець присутній» для двох `.lisp` сценаріїв:

- `scripts/scenario-log.lisp` викликає `запиши-лог` рівно один раз;
- `scripts/scenario-silent.lisp` не викликає його жодного разу.
