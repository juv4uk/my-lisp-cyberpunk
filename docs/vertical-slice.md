...
6. обчислює `(запиши-лог)`, звіряє результат з oracle (`()`), fails closed
   при розбіжності;
7. обчислює `(гравець-присутній?)` **повторювано** — через `OnUpdate` для
   `EGameStateType::Running`, не одноразовий `OnEnter` — поки не побачить
   `t` (RED4ext ігнорує повернене значення `OnUpdate` саме для `Running`,
