...
| --- | --- | --- | --- |
| C++ намагався викликати compiled Redscript UI через `CRTTISystem::GetFunction`. | Цей registry придатний для native `CGlobalFunction` і native class methods; compiled Redscript presentation не є таким global function. | Функція UI не була знайдена у native registry. | C++ більше не викликає Redscript presentation напряму. |
...
| F10 тримали в C++ як host input fact. | Це вимагало двох нестабільних мостів: `UISystem.QueueEvent` і прямого RTTI-виклику compiled Redscript function. Обидва дійшли до плагіна, але не показали UI. | Live log зафіксував `NeuralDeckService.ToggleFromNative missing`, а встановлений Codeware і робочий Codeware-мод показують прямий `CallbackSystem.RegisterCallback("Input/Key", ...)` шлях. | F10 належить Codeware callback-системі у `NeuralDeckService`; C++ не опитує клавіатуру і не керує presentation. |
...
2. `CallbackSystem.RegisterCallback` повертає `CallbackSystemHandler`, який має `AddTarget` і `Unregister`;
3. `InputTarget.Key(EInputKey.IK_F10, EInputAction.IACT_Press)` є штатним Codeware filter для F10;
4. `CustomPopupManager.ShowPopup` створює і прикріплює popup у власному Redscript lifecycle;
5. робочий Codeware-мод застосовує цей самий `Input/Key` callback pattern для гарячих клавіш.

Звідси випливає поточний міст:

```text
Codeware Input/Key
  -> NeuralDeckService.OnNeuralDeckKey
  -> Codeware CustomPopup
```

Цей міст не передає Lisp semantics у C++, не створює другого evaluator і не
перетворює C++ на власника UI policy. Redscript/Codeware володіє клавішею та
presentation lifecycle; C++ лишається host-механізмом для Lisp.
...
5. Redscript log має показати `registered Codeware F10 callback`, `received
   Codeware F10 press` та результат `ToggleOverlay`.
...
Поточна реалізація зібрана і пройшла CTest. Вона ще не має live evidence,
що Codeware доставив F10 у `NeuralDeckService` і popup прикріпився. До появи
такого transcript статус F10 — **in progress**, а не done.
