...
він реєструє українські host-capabilities: `(запиши-лог)`,
`(гравець-присутній?)` і read-only `(клас гравець)`. Сценарій у
`scripts/dispatcher.lisp` вирішує, що викликати; adapter лише виконує
дію й пише результат Lisp-dispatch у RED4ext log. Збереження, інвентар,
позиція гравця та RTTI-об'єкти не змінюються.
...
`scripts/dispatcher.lisp`, завантажений поряд із DLL під час plugin Load. У цьому
...
`dispatcher.lisp` і перезавантаження plugin змінює поведінку без зміни C++.
...
а `scenario-log.lisp` викликає log один раз, коли `scenario-silent.lisp` — жодного.
