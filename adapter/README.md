...
`scripts/диспетчер.my`, завантажений поряд із DLL під час plugin Load. У цьому
...
`диспетчер.my` і перезавантаження plugin змінює поведінку без зміни C++.
...
а `сценарій-лог.my` викликає log один раз, коли `сценарій-тиша.my` — жодного.
