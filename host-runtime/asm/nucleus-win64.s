 ...
 *
 * Fail-closed: Tag::Cons is zero, so any word with a nonzero TAG_MASK bit
 * is not a pair and must not be dereferenced. Ported from wsm-my-lisp
 * commit fdf9281 (found there: this repo's copy forked at Phase C and no
 * longer syncs, so the same real UB -- reading arbitrary memory at a
 * shifted Fixnum, or dereferencing a null Nil pointer -- existed here
 * too, unnoticed until now). my-lisp's oracle requires (car 5) and
