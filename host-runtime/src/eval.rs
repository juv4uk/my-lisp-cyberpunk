...
// registry id `0001`).
//
// First fix (commit `c3bf0f4`) hardcoded the known spellings as a
// `matches!` list -- correct in outcome, but the owner's own follow-up
// review named it a new instance of exactly the copy-paste-drift
// problem `wsm-my-lisp#4` exists to prevent (these spellings are a
// semantic fact my-lisp owns, not wsm-my-lisp's to fork by hand). Fixed
// for real here: `QUOTE_SPELLINGS`/`COND_SPELLINGS` are generated by
// `build.rs`, which parses `semantic-registry.wsm` directly at build
// time -- see that file's own module doc for the parser. If the
// registry's spellings for either entry change, a `cargo build` alone
// regenerates this; nothing in eval.rs needs hand-editing.
...
mod tests {
    ...
    fn nested_call_argument_evaluates_before_dispatch() {
        ...
        let word = read_one("(teleport player (+ x 10) 200 -10)", &mut symbols, &mut strings).unwrap();
        eval(word, &env, &symbols).unwrap();
        let recorded = calls.borrow();
        assert_eq!(
